...
### Adding settings
First, see the [Usage section of the laravel-settings](https://github.com/spatie/laravel-settings?tab=readme-ov-file#usage) package to create a new settings class.  
...
This method also allows better content separation, so that a translator or editor can be given permission to translate stuff without having permission to change app/plugin config, which is usually only done by super-admins.
